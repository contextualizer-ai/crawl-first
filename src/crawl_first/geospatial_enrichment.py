...
try:
    from .crosswalk_loader import normalize_usda_texture, normalize_osm_natural
except ImportError:
    print("Warning: crosswalk_loader not available - linked data normalization disabled")
...
class GeospatialCache:
    ...
    def __init__(self, cache_dir: str = None):
        if cache_dir is None:
            ...
            cache_dir = str(Path(__file__).parent.parent.parent / "cache" / "geospatial")
    ...
    def _cache_key(self, api_name: str, params: Dict[str, Any]) -> str:
        ...
        precision = self.GEOHASH_PRECISION.get(api_name, self.GEOHASH_PRECISION["default"])
    ...
    def set(self, api_name: str, params: Dict[str, Any], result: Dict[str, Any]) -> None:
        ...
        cache_data = {
            ...
            "ttl_seconds": self.DATASET_TTLS.get(api_name, self.DATASET_TTLS["default"]),
            "data": result
        }
    ...
    def _has_enhanced_metadata(self, api_name: str, data: Dict[str, Any]) -> bool:
        ...
        if api_name == "weather":
            ...
            return (
                "coordinate_distance_km" in data or 
                "coordinate_metadata" in data or
                "actual_data_coordinates" in data
            )
        return True  # Other APIs don't need special enhanced metadata checks
    
    def get_cached_data(self, api_name: str, params: Dict[str, Any]) -> Optional[Dict[str, Any]]:
...
def get_elevation_open_elevation(lat: float, lon: float) -> Dict[str, Any]:
    ...
    try:
        ...
        # Cache the result
        _cache.set("elevation", cache_params, result)
        return result
...
def add_weather_station_distance_metadata(sample_lat: float, sample_lon: float, 
                                        station_lat: float, station_lon: float,
                                        station_name: str = "Unknown Station") -> Dict[str, Any]:
    """
    ...
    return {
        "station_info": {
            ...
            "formatted_coordinates": validate_coordinates(station_lat, station_lon)["metadata"]["formatted"]
        },
        "sample_coordinates": {"lat": sample_lat, "lon": sample_lon},
        "distance_analysis": {
            "distance_km": round(distance_km, 2),
            "quality_rating": quality_rating,
            "representativeness": "high" if distance_km < 25 else "moderate" if distance_km < 100 else "low"
        },
        "data_quality_notes": [
...
def get_weather_data_open_meteo(lat: float, lon: float, date: str) -> Dict[str, Any]:
    ...
    # Comprehensive daily weather parameters
    daily_params = [
        # Temperature (°C)
        ...
        "dewpoint_2m_mean", "soil_temperature_0cm_mean",
    ...
    try:
        ...
        if "daily" in data and data["daily"]:
            ...
            # Get the actual coordinates returned by Open-Meteo (may differ from requested)
            ...
            # Validate coordinates and add metadata
            ...
            result = {
                ...
                "actual_data_coordinates": {"latitude": returned_lat, "longitude": returned_lon},
                ...
                "coordinate_metadata": {
                    ...
                    "distance_analysis": {
                        ...
                        "representativeness": "high" if coord_distance_km < 25 else "moderate" if coord_distance_km < 100 else "low"
                    },
                    "formatted_requested": coord_validation["metadata"]["formatted"],
                    "formatted_actual": returned_coord_validation["metadata"]["formatted"],
                    "data_source_type": "adjusted_coordinates",
                    "spatial_resolution": "Open-Meteo grid point or station location",
                    "coordinate_note": f"Open-Meteo adjusted coordinates by {coord_distance_km:.2f}km"
                },
                "data_quality": {
                    "source_type": "grid_point_or_station",
                    "confidence": distance_quality["rating"],
                    "distance_quality_score": distance_quality["score"],
                    "notes": [
                        f"Open-Meteo returned data for coordinates {coord_distance_km:.2f}km from requested location",
                        f"Data quality: {distance_quality['description']}",
                        "Coordinates may represent nearest grid point or weather station"
                    ]
                },
                "success": True
            }
    ...
    for category, values in OSM_ENVIRONMENTAL_TAGS.items():
        if values:  # Only add if category has values
            for value in values:
                ...
                query_parts.append(f'way["{category}"="{value}"](around:{radius_m},{lat},{lon});')
                query_parts.append(f'relation["{category}"="{value}"](around:{radius_m},{lat},{lon});')
                # Add nodes for point features (especially for natural features)
                if category in ["natural", "man_made", "amenity"]:
                    query_parts.append(f'node["{category}"="{value}"](around:{radius_m},{lat},{lon});')
...
def get_nearby_features_overpass(lat: float, lon: float, radius_km: float = 1.0) -> Dict[str, Any]:
    ...
    try:
        response = requests.post(url, data=query, timeout=120)  # Increased timeout for comprehensive query
        response.raise_for_status()
        data = response.json()
            ...
            for element in data["elements"][:50]:
                ...
                if tags:
                    ...
                    for category in OSM_ENVIRONMENTAL_TAGS.keys():
                        if category in tags and tags[category] in OSM_ENVIRONMENTAL_TAGS[category]:
        ...
        result = {
            ...
            "query_metadata": {
                ...
                "timeout_seconds": 120,
                "query_type": "comprehensive_environmental" if COMPREHENSIVE_OSM_AVAILABLE else "basic_fallback"
            },
...
def get_soil_classification_nrcs_sda(lat: float, lon: float) -> Dict[str, Any]:
    ...
    try:
        ...
        r_schema = requests.post(SDA_URL, data={
            "SERVICE": "query", "REQUEST": "query", "FORMAT": "JSON", "QUERY": q_schema
        ...
        if r_schema.status_code == 200:
            ...
            if schema_data.get('Table'):
                ...
                available_columns = {row[1].lower() for row in schema_data['Table'] if len(row) > 1}
        ...
        safe_tax_cols = [col for col in desired_tax_cols if col in available_columns] if available_columns else ["taxorder", "taxsuborder", "taxsubgrp"]
        
        # Step 1: Get mapunit key(s) at point (WGS84)  
        wkt = f"POINT({lon} {lat})"
        q1 = f"SELECT * FROM SDA_Get_Mukey_from_intersection_with_WktWgs84('{wkt}')"
        r1 = requests.post(SDA_URL, data={
            "SERVICE": "query", "REQUEST": "query", "FORMAT": "JSON", "QUERY": q1
        }, headers=HDRS, timeout=30)
        r1.raise_for_status()
        data = r1.json()
        ...
        r2 = requests.post(SDA_URL, data={
            "SERVICE": "query", "REQUEST": "query", "FORMAT": "JSON", "QUERY": q2
        }, headers=HDRS, timeout=30)
        r2.raise_for_status()
        comp_data = r2.json()
        ...
        if comp_data and 'Table' in comp_data and comp_data['Table']:
            ...
            result = {
                ...
                "schema_safe": True,
                "available_columns": len(available_columns) if available_columns else "schema_query_failed",
                "success": True
            }
        else:
            result = {
        ...
        if data.get('wrb_class_name'):
            ...
            result = {
                "soil_classification": {
                    ...
                    "primary_probability": data.get('wrb_class_probability', [{}])[0][1] if data.get('wrb_class_probability') else None,
...
def soilgrids_wcs_point(lon: float, lat: float, coverage_id: str = "phh2o_0-5cm_Q0.5", 
                        map_path: str = "/map/phh2o.map", timeout: int = 30) -> Optional[bytes]:
    """
...
def get_soil_properties_isric_wcs(lat: float, lon: float) -> Dict[str, Any]:
    ...
    soil_maps = [
        {"name": "ph_0_5cm", "map": "/map/phh2o.map", "coverage": "phh2o_0-5cm_Q0.5", "scale": 0.1, "unit": "pH"},        # pH×10 → pH
        {"name": "soc_0_5cm", "map": "/map/soc.map", "coverage": "soc_0-5cm_mean", "scale": 0.1, "unit": "g/kg"},         # dg/kg → g/kg  
        {"name": "sand_0_5cm", "map": "/map/sand.map", "coverage": "sand_0-5cm_mean", "scale": 0.1, "unit": "%"},         # g/kg → %
        {"name": "silt_0_5cm", "map": "/map/silt.map", "coverage": "silt_0-5cm_mean", "scale": 0.1, "unit": "%"},         # g/kg → %
        {"name": "clay_0_5cm", "map": "/map/clay.map", "coverage": "clay_0-5cm_mean", "scale": 0.1, "unit": "%"},         # g/kg → %
        {"name": "bdod_0_5cm", "map": "/map/bdod.map", "coverage": "bdod_0-5cm_mean", "scale": 0.01, "unit": "g/cm³"},    # cg/cm³ → g/cm³
        {"name": "nitrogen_0_5cm", "map": "/map/nitrogen.map", "coverage": "nitrogen_0-5cm_mean", "scale": 0.01, "unit": "g/kg"},  # cg/kg → g/kg
        {"name": "ocd_0_5cm", "map": "/map/ocd.map", "coverage": "ocd_0-5cm_mean", "scale": 0.1, "unit": "kg/dm³"},       # hg/dm³ → kg/dm³
        {"name": "ocs_0_30cm", "map": "/map/ocs.map", "coverage": "ocs_0-30cm_mean", "scale": 0.1, "unit": "kg/m²"}       # t/ha×10 → kg/m²
    ]
        ...
        try:
            ...
            if tif_bytes:
                ...
                if pixel_value is not None:
                    ...
                    properties[prop["name"]] = {"value": None, "unit": prop["unit"], "error": "No data pixel"}
            else:
                properties[prop["name"]] = {"value": None, "unit": prop["unit"], "error": "WCS request failed"}
        except Exception as e:
            properties[prop["name"]] = {"value": None, "unit": prop["unit"], "error": str(e)}
    ...
    if sand_value is not None and clay_value is not None:
        try:
            ...
            calculated_silt = 100 - sand_value - clay_value if silt_value is None else silt_value
            ...
            if crosswalk_result:
                texture_classification["raw_classification"] = crosswalk_result["raw_classification"]
...
def classify_ecosystem_v0(lat: float, lon: float, elevation_data: Dict[str, Any], 
                         ecoregion_data: Dict[str, Any], nearby_features: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    features["is_marine_realm"] = realm and "marine" in realm.lower() if realm else False
    ...
    if nearby_features.get("success") and nearby_features.get("features"):
        for feature in nearby_features["features"]:
            # Check natural tag for water features
            natural_tag = feature.get("natural", "").lower()
            water_tag = feature.get("water", "").lower()
            name = feature.get("name", "").lower()
            
            # Water feature detection
            water_indicators = ["water", "river", "lake", "stream", "pond", "reservoir", "spring"]
            if any(indicator in natural_tag for indicator in water_indicators):
                water_features.append(natural_tag)
                
            # Freshwater detection - lakes, rivers, streams, springs are typically freshwater
            freshwater_indicators_list = ["lake", "river", "stream", "pond", "spring", "hot_spring"]
            if any(indicator in natural_tag for indicator in freshwater_indicators_list):
                freshwater_indicators.append(natural_tag)
                
            # Also check water tag
            if water_tag in ["lake", "pond", "reservoir", "river"]:
                freshwater_indicators.append(water_tag)
    ...
    features["in_potential_eez"] = (
        (lat > -60 and lat < 80) and  # Exclude Antarctica and extreme Arctic
        (abs(lat) < 75)  # Exclude most polar regions
    )
    ...
    # Rule 2: Oceanic
    if features["is_below_sea_level"] or (features["is_near_sea_level"] and features["is_marine_realm"]):
    ...
    elif features["is_near_sea_level"] and features["has_nearby_water"] and not features["freshwater_nearby"]:
        ecosystem_path = "nearshore"
        confidence = 0.7
        reasoning.append("Near sea level with nearby water features (non-freshwater)")
    ...
    # Rule 6: Default terrestrial
        ...
        ecosystem_path = "terrestrial"
        confidence = 0.3
        reasoning.append("Default classification - insufficient features for higher confidence")
...
def enrich_location(lat: float, lon: float, collection_date: Optional[str] = None) -> Dict[str, Any]:
    ...
    if -170 <= lon <= -60 and 15 <= lat <= 75:  # US bounds
        ...
        if not soil_classification.get("success"):
            soil_classification_global = get_soil_classification_isric_soilgrids(lat, lon)
