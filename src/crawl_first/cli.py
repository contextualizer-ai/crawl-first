...
def str_presenter(dumper, data):
    ...
    if '\n' in data or data.startswith('%') or any(char in data for char in [':', '{', '}', '[', ']', ',', '#', '&', '*', '!', '|', '>', "'", '"', '`']):
        return dumper.represent_scalar('tag:yaml.org,2002:str', data, style='"')
    return dumper.represent_scalar('tag:yaml.org,2002:str', data)

NoRefsDumper.add_representer(str, str_presenter)
from nmdc_mcp.api import fetch_nmdc_entity_by_id_with_projection, fetch_nmdc_entity_by_id
...
from artl_mcp.tools import (
    doi_to_pmid, get_doi_metadata, get_abstract_from_pubmed_id, extract_paper_info,
    get_full_text_from_doi, get_doi_text, get_pmid_text, get_pmcid_text, 
    extract_pdf_text, get_unpaywall_info
)
...
OSM_ENVIRONMENTAL_TAGS = {
    "natural": {
        # Water features
        "water", "coastline", "bay", "cape", "peninsula", "strait", "fjord",
        "spring", "hot_spring", "geyser", "mineral_spring",
        # Vegetation
        "wood", "tree", "tree_row", "grassland", "scrub", "heath", "moor",
        "fell", "tundra", "shrubbery",
        # Wetlands
        "wetland", "marsh", "swamp", "bog", "fen", "wet_meadow", "reedbed", "saltmarsh",
        # Geological features
        "beach", "sand", "shingle", "bare_rock", "scree", "cliff", "rock", "stone", "boulder",
        "peak", "ridge", "valley", "saddle", "col", "spur", "arete",
        "cave_entrance", "cave", "sinkhole", "karst",
        # Volcanic/thermal
        "volcano", "crater", "lava_field", "fumarole", "mud",
        # Ice/snow
        "glacier", "snowfield", "ice_shelf",
        # Desert features
        "desert", "dune", "oasis"
    },
    "landuse": {
        # Agricultural
        "forest", "farmland", "meadow", "orchard", "vineyard", "agricultural",
        "grass", "pasture", "allotments", "plant_nursery",
        # Conservation
        "nature_reserve", "conservation", "greenfield",
        # Water management
        "wetland", "water", "reservoir", "salt_pond", "aquaculture", "basin",
        # Recreation
        "recreation_ground", "village_green", "cemetery",
        # Industrial/extractive
        "quarry", "landfill", "brownfield", "industrial", "commercial",
        # Waste management
        "dump", "waste_disposal", "scrap_yard", "salvage_yard",
        # Other
        "floodplain"
    },
    "waterway": {
        "river", "stream", "canal", "drain", "ditch", "brook", "creek",
        "rapids", "waterfall", "dam", "weir", "lock_gate", "dock",
        "tidal_channel", "intermittent"
    },
    "water": {
        "lake", "pond", "reservoir", "river", "stream", "canal", "lagoon",
        "bay", "wetland", "marsh", "swamp", "salt_water", "fresh_water",
        "oxbow", "reflecting_pool", "wastewater", "intermittent"
    },
    "wetland": {
        "marsh", "swamp", "bog", "fen", "wet_meadow", "reedbed", "saltmarsh",
        "tidalflat", "saltern", "mangrove"
    },
    "leisure": {
        "nature_reserve", "park", "garden", "common", "recreation_ground",
        "dog_park", "picnic_site", "beach_resort", "fishing"
    },
    "boundary": {
        "national_park", "protected_area", "nature_reserve", "marine_protected_area",
        "aboriginal_lands", "indigenous_territory"
    },
    "place": {
        "island", "islet", "archipelago", "atoll", "reef"
    },
    "geological": {
        "outcrop", "moraine", "ridge", "valley"
    },
    "man_made": {
        # Water infrastructure
        "pier", "breakwater", "groyne", "dyke", "embankment", "dam",
        "reservoir_covered", "water_tower", "water_well", "wastewater_plant",
        # Monitoring/research
        "monitoring_station", "weather_station", "research_station",
        # Waste management
        "waste_disposal", "wastewater_plant", "recycling"
    },
    "amenity": {
        "research_station", "waste_disposal", "recycling", "waste_transfer_station"
    },
    "highway": {
        "track", "path", "footway", "bridleway"  # Can indicate human impact/access
    },
    "barrier": {
        "fence", "wall", "hedge"  # Can indicate land use boundaries
    },
    "building": {
        # Agricultural
        "barn", "farm", "farm_auxiliary", "greenhouse", "silo", "stable", "cowshed",
        "chicken_coop", "livestock", "slurry_tank",
        # Research/Educational
        "university", "college", "research", "laboratory", "observatory",
        # Industrial/Processing
        "industrial", "warehouse", "factory", "refinery", "power_plant",
        "water_treatment", "wastewater_treatment", "sewage_treatment",
        # Mining/Extractive
        "mine", "quarry_office", "mining",
        # Storage/Hazardous
        "fuel_storage", "storage_tank", "hazmat", "chemical_storage",
        # Utilities
        "pumping_station", "substation", "utility"
    },
    "amenity": {
        "research_station", "waste_disposal", "recycling", "waste_transfer_station",
        # Water/waste management
        "waste_basket", "toilets", "drinking_water", "water_point",
        "wastewater_plant", "fuel", "charging_station",
        # Research/education
        "university", "college", "research_institute", "library",
        # Agriculture related
    ...
    "industrial": {
        "factory", "refinery", "mine", "quarry", "oil_well", "gas_well",
        "chemical", "petroleum", "wastewater_treatment", "water_treatment",
        "power", "solar_park", "wind_farm"
    },
    "power": {
        "plant", "generator", "substation", "transformer", "solar_panel",
        "wind_turbine", "geothermal"
    }
}
...
def cached_search_all_ontologies(query: str, ontologies: str = "envo", max_results: int = 2, exact: bool = False) -> List[Dict[str, Any]]:
    """Cached wrapper for OLS ontology search."""
    key = cache_key({"query": query.lower(), "ontologies": ontologies, "max_results": max_results, "exact": exact})
    cached = get_cache("ols_search", key)
    if cached:
        return cached.get("results", [])
    
    try:
        results = search_all_ontologies(query=query, ontologies=ontologies, max_results=max_results, exact=exact)
...
def cached_fetch_nmdc_entity_by_id(entity_id: str) -> Optional[Dict[str, Any]]:
    ...
    cached = get_cache("nmdc_entity", key)
    if cached:
...
def cached_fetch_nmdc_entity_by_id_with_projection(entity_id: str, collection: str, projection: List[str]) -> Optional[Dict[str, Any]]:
    """Cached wrapper for NMDC entity fetch with projection."""
    key = cache_key({"entity_id": entity_id, "collection": collection, "projection": sorted(projection)})
    cached = get_cache("nmdc_entity_projection", key)
    if cached:
        return cached.get("entity")
    
    try:
        entity = fetch_nmdc_entity_by_id_with_projection(entity_id=entity_id, collection=collection, projection=projection)
...
def cached_get_study_for_biosample(biosample_id: str) -> Dict[str, Any]:
    ...
    cached = get_cache("study_for_biosample", key)
    if cached:
...
def cached_get_study_doi_details(study_id: str) -> Dict[str, Any]:
    ...
    cached = get_cache("study_doi_details", key)
    if cached:
        return cached.get("result", {})
...
def save_full_text_to_file(content: str, identifiers: Dict[str, str]) -> Optional[str]:
    ...
    try:
        ...
        if identifiers.get('doi'):
            ...
            clean_doi = identifiers['doi'].replace('/', '_').replace(':', '_').replace('.', '_')
        ...
        filename = "_".join(filename_parts[:2]) + ".txt"  # Limit to 2 parts to avoid overly long names
...
def cached_get_full_text(paper_metadata: Dict[str, Any], email: str) -> Optional[Dict[str, Any]]:
    """
    ...
    try:
        ...
        if doi:
            try:
                ...
                if text and len(text.strip()) > 100:
                    attempts["artl_doi_simple"] = {"text": text, "length": len(text.strip())}
            except Exception:
                pass
        ...
        if doi:
            try:
                ...
                if text and len(text.strip()) > 100:
                    attempts["artl_doi_advanced"] = {"text": text, "length": len(text.strip())}
            except Exception:
                pass
                ...
                if unpaywall_info and unpaywall_info.get('is_oa'):
                    ...
                    for location in oa_locations:
                        ...
                        if pdf_url:
                            ...
                            if text and len(text.strip()) > 100:
                                attempts["artl_unpaywall_pdf"] = {"text": text, "length": len(text.strip())}
                                break
            except Exception:
                pass
        ...
        if full_text and len(full_text.strip()) > 100:  # Only save if substantial content
        ...
        result = {
            ...
            "status": "retrieved" if full_text and file_path else ("partial" if full_text else "not_available"),
            "methods_attempted": list(attempts.keys()) if attempts else [],
            "method_results": {k: {"length": v["length"]} for k, v in attempts.items()} if attempts else {}
        }
...
def find_closest_landuse_date(target_date: str, available_dates: List[str]) -> Optional[str]:
    """
...
def get_soil_analysis(lat: float, lon: float) -> Dict[str, Any]:
    ...
    try:
        ...
        if soil_type:
            try:
                matches = cached_search_all_ontologies(query=soil_type, ontologies="envo", max_results=2, exact=False)
                if matches:
...
def get_land_cover_analysis(lat: float, lon: float, date: str) -> Dict[str, Any]:
    ...
    key = cache_key({"lat": round(lat, 4), "lon": round(lon, 4), "date": actual_start_date})
    cached = get_cache("land_cover", key)
    if cached:
        return cached
    
    try:
        land_cover = get_land_cover(lat=lat, lon=lon, start_date=actual_start_date, end_date=actual_end_date)
        ...
        if land_cover:
            ...
            for term in unique_terms:
                try:
                    matches = cached_search_all_ontologies(query=term, ontologies="envo", max_results=2, exact=False)
                    if matches:
                        # Clean up matches - keep entries with valid obo_id
                        cleaned_matches = []
                        for match in matches[:2]:  # Only take top 2 most relevant
...
def get_nearby_year_date(date_str: str, year_offset: int) -> Optional[str]:
    """Get a date offset by specified years."""
    try:
        dt = datetime.strptime(date_str, '%Y-%m-%d')
...
def get_weather_analysis(lat: float, lon: float, date: str) -> Optional[Dict[str, Any]]:
    ...
    strategies = [
        ...
        # Strategy 3: Nearby dates (±1 year), close radius
        {"search_radius_km": 75, "coverage_threshold": 0.25, "date": get_nearby_year_date(parsed_date, -1)},
        {"search_radius_km": 75, "coverage_threshold": 0.25, "date": get_nearby_year_date(parsed_date, 1)},
        # Strategy 4: Last resort - wider but still reasonable
        {"search_radius_km": 150, "coverage_threshold": 0.1, "date": parsed_date}
    ]
        ...
        try:
            ...
            if weather_data:
                ...
                if "station" in weather_data:
                    ...
                    cleaned["station"] = {k: v for k, v in station.items() 
                                        if k in ["name", "country", "region", "wmo", "icao", 
                                               "latitude", "longitude", "elevation", "timezone", "distance"]}
                        ...
                        if isinstance(values, dict) and values:
                            for timestamp, value in values.items():
                                if value is not None and not (isinstance(value, float) and str(value) == 'nan'):
                                    ...
                                    if measurement_type in ['tavg', 'tmin', 'tmax'] and isinstance(value, (int, float)):
                                        value = round(value - 273.15, 1)  # Kelvin to Celsius
                                    measurements[measurement_type] = value
                                    break
                    cleaned["measurements"] = measurements
                
                save_cache("weather", key, cleaned)
                return cleaned
...
def get_publication_analysis(biosample_id: str, email: str) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        if "error" in study_result or not study_result.get("study_id"):
            save_cache("publications", key, None)
            return None
        ...
        study_data = cached_fetch_nmdc_entity_by_id_with_projection(
            entity_id=study_id, collection="study_set",
            projection=[
                "id", "name", "title", "description", "funding_sources", "websites", "associated_dois",
                "protocol_link", "objective", "notes", "homepage_website", 
                "alternative_titles", "alternative_names", "alternative_descriptions"
            ]
        )
        ...
        if study_data and isinstance(study_data, dict) and "error" not in study_data:
            study_info.update({
                ...
                "alternative_descriptions": study_data.get("alternative_descriptions", [])
            })
        
        # Process DOIs
        all_dois = study_data.get("associated_dois", []) if study_data else doi_details.get("associated_dois", [])
        enhanced_dois = []
        ...
        for doi_entry in all_dois:
            ...
            if doi_value:
                ...
                if doi_category == "publication_doi":
                    ...
                    # Get paper metadata
                    paper_metadata = {"doi": clean_doi, "pmid": pmid, "title": "", "authors": [], 
                                    "journal": "", "citation_count": None, "abstract": "", "retrieval_errors": []}
                    ...
                    try:
                        ...
                        paper_metadata["retrieval_errors"].append(f"CrossRef error: {e}")
                    ...
                    if pmid and not paper_metadata.get("abstract"):
                        try:
                            ...
                            paper_metadata["retrieval_errors"].append(f"PubMed error: {e}")
                    ...
                    enhanced_doi.update({
                        "converted_ids": {"original_doi": doi_value, "pmid": pmid, "pmcid": None},
        ...
        result = {
            ...
            "award_doi_count": len([doi for doi in enhanced_dois if doi.get("doi_category") == "award_doi"]),
            "dataset_doi_count": len([doi for doi in enhanced_dois if doi.get("doi_category") == "dataset_doi"])
        }
        
        # Store all_dois separately so it can be extracted as a sibling
        result["_all_dois"] = enhanced_dois
        
        save_cache("publications", key, result)
        return result
...
def geocode_location_name(location_name: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if location:
            ...
            address_fields = [
                "house_number", "road", "neighbourhood", "suburb", "village", 
                "town", "city", "municipality", "county", "state", "country",
    ...
    try:
        ...
        if location:
            ...
            for field in admin_fields:
                if field in address:
                    result["administrative"][field] = address[field]
...
def get_feature_type(tags: Dict[str, str]) -> Optional[str]:
    ...
    categories = [
        "natural", "water", "wetland", "waterway", "geological", 
        "landuse", "leisure", "boundary", "place", "man_made", 
        "building", "industrial", "power", "amenity", "highway", "barrier"
    ]
    
    for category in categories:
        if category in tags and tags[category] in OSM_ENVIRONMENTAL_TAGS.get(category, set()):
...
def extract_coordinates(element: Dict) -> Tuple[float, float]:
    ...
    if "center" in element:
        ...
        raise ValueError(f"No coordinates found in element")


def query_osm_features(lat: float, lon: float, radius: float = 1000) -> List[OSMFeature]:
        ...
        try:
            ...
            for element in data["elements"]:
                try:
                    ...
                    distance = calculate_distance(lat, lon, coordinates[0], coordinates[1])
...
def summarize_osm_features(features: List[OSMFeature], center_lat: float, center_lon: float) -> Dict[str, Any]:
    ...
    for f in sorted_features[:5]:
        ...
        nearest_features.append(feature_info)
    
    summary["nearest_features"] = nearest_features
    
    for feature in features:
        category = feature.feature_type.split(":")[0]
        feature_type = feature.feature_type.split(":")[1]
        
        if category not in summary["metadata"]["feature_counts"]:
            summary["metadata"]["feature_counts"][category] = {}
        if feature_type not in summary["metadata"]["feature_counts"][category]:
            summary["metadata"]["feature_counts"][category][feature_type] = 0
        summary["metadata"]["feature_counts"][category][feature_type] += 1
        
        # Only include features that have names (skip unnamed features)
        name = feature.tags.get("name")
        if name and name != "Unnamed":
            if category not in summary["environmental_features"]:
                summary["environmental_features"][category] = []
            ...
            relevant_tags = {k: v for k, v in feature.tags.items() 
            ...
            summary["environmental_features"][category].append(feature_info)
        
        # Count ALL features for environmental summary (including unnamed ones)
        if category in ["water", "waterway"] or feature_type in ["water", "lake", "pond", "river", "stream"]:
            water_features_temp.append(1)  # Just count, don't store data
            
        if category == "natural" or feature_type in ["forest", "wood", "grassland", "wetland"]:
            natural_areas_temp.append(1)  # Just count, don't store data
            
        if feature_type in ["nature_reserve", "national_park", "protected_area", "conservation"]:
    ...
    for category in summary["environmental_features"]:
        # Sort by distance first
        summary["environmental_features"][category].sort(key=lambda x: x["distance_m"])
        
        # Deduplicate by name - keep only the closest (first) occurrence
        seen_names = set()
        unique_features = []
        for feature in summary["environmental_features"][category]:
            name = feature["name"]
            if name not in seen_names:
                seen_names.add(name)
                unique_features.append(feature)
        
        summary["environmental_features"][category] = unique_features
    
    return summary


def get_geospatial_analysis(lat: float, lon: float, radius: float = 1000) -> Dict[str, Any]:
    ...
    if eco_description:
        analysis["environmental_summary"]["ecological_context"] = (
...
def analyze_biosample(biosample_id: str, email: str, search_radius: int = 1000) -> Optional[Dict[str, Any]]:
    ...
    try:
        ...
        location_name = geo_loc_name.get("has_raw_value") if isinstance(geo_loc_name, dict) else None
        
        collection_date = full_biosample.get("collection_date", {})
        date = collection_date.get("has_raw_value") if isinstance(collection_date, dict) else None
        parsed_date = parse_collection_date(date) if date else None
        ...
        if asserted_lat is not None and asserted_lon is not None:
            ...
            inferred["soil_from_asserted_coords"] = get_soil_analysis(asserted_lat, asserted_lon)
            
            # Land cover analysis from asserted coords
            if date:
                inferred["land_cover_from_asserted_coords"] = get_land_cover_analysis(asserted_lat, asserted_lon, date)
                
                # Available landuse dates (show matched date + neighbors)
                if parsed_date:
                    try:
                        dates_key = cache_key({"lat": round(asserted_lat, 4), "lon": round(asserted_lon, 4)})
                        cached_dates = get_cache("landuse_dates", dates_key)
                        if not cached_dates:
                            cached_dates = get_landuse_dates(lat=asserted_lat, lon=asserted_lon)
            ...
            geospatial_asserted = get_geospatial_analysis(asserted_lat, asserted_lon, radius=search_radius)
        ...
        if location_name and location_name.strip():
            ...
            if geocode_result.get("coordinates") and not geocode_result.get("error"):
                ...
                if elevation_geocoded is not None:
                    coord_sources["from_geo_loc_name"]["elevation_meters"] = elevation_geocoded
        ...
        if coord_sources:
            ...
            if "from_asserted_coords" in coord_sources and "from_geo_loc_name" in coord_sources:
                ...
                asserted_lat, asserted_lon = asserted_coords["latitude"], asserted_coords["longitude"]
                geocoded_lat, geocoded_lon = geocoded_coords["latitude"], geocoded_coords["longitude"]
                
                # Calculate distance between asserted and geocoded coordinates
                distance_meters = calculate_distance(asserted_lat, asserted_lon, geocoded_lat, geocoded_lon)
            ...
            asserted_elevation = full_biosample.get("elev")  # Check if biosample has asserted elevation
            if (asserted_elevation is not None and 
                "geospatial_from_asserted_coords" in inferred and 
                inferred["geospatial_from_asserted_coords"].get("elevation")):
                
                geospatial_elevation = inferred["geospatial_from_asserted_coords"]["elevation"]["meters"]
...
@click.option('--input-file', type=click.Path(exists=True), help='File with biosample IDs (one per line)')
@click.option('--sample-size', type=int, help='Random sample size from input file')
@click.option('--output-file', type=click.Path(), help='Output YAML file (single file for all results)')
@click.option('--output-dir', type=click.Path(), help='Output directory (individual YAML files per biosample)')
@click.option('--email', required=True, help='Email address for API requests (required for full text fetching)')
@click.option('--search-radius', type=int, default=1000, help='Search radius in meters for geospatial features (default: 1000)')
@click.option('--verbose', is_flag=True, help='Verbose output')
def main(biosample_id: str, input_file: str, sample_size: int, output_file: str, output_dir: str, email: str, search_radius: int, verbose: bool):
    """crawl-first: Deterministic biosample enrichment for LLM-ready data preparation."""
    
    # Validate input options
    if input_file and biosample_id:
        click.echo("Error: Cannot specify both --biosample-id and --input-file", err=True)
        return
    elif not input_file and not biosample_id:
        click.echo("Error: Must specify either --biosample-id or --input-file", err=True)
        return
    
    # Validate output options
    if output_file and output_dir:
        click.echo("Error: Cannot specify both --output-file and --output-dir", err=True)
        return
    elif not output_file and not output_dir:
        click.echo("Error: Must specify either --output-file or --output-dir", err=True)
        return
    ...
    if input_file:
        ...
        if not biosample_ids:
            click.echo("No biosample IDs to process", err=True)
            return
    else:
            ...
            if output_dir:
                ...
                try:
                    with open(file_path, 'w') as f:
                        yaml.dump(result, f, Dumper=NoRefsDumper, default_flow_style=False, sort_keys=False, indent=2)
                    if verbose:
    ...
    if len(biosample_ids) > 1:
        if output_dir:
            ...
            click.echo(f"Processed {successful_count}/{len(biosample_ids)} biosamples successfully")
            click.echo(f"Files saved to: {output_dir}")
        else:
            click.echo(f"Processed {len(results)}/{len(biosample_ids)} biosamples successfully")
    ...
    if output_file and results:
        ...
        with open(output_file, 'w') as f:
            yaml.dump(output_data, f, Dumper=NoRefsDumper, default_flow_style=False, sort_keys=False, indent=2)
        ...
        if len(results) == 1:
            ...
            if "from_asserted_coords" in coord_sources:
                ...
                click.echo(f"Asserted coordinates: {coords['latitude']}, {coords['longitude']}")
            ...
            if "from_geo_loc_name" in coord_sources:
                ...
                click.echo(f"Geocoded coordinates: {coords['latitude']}, {coords['longitude']} ({source})")
            ...
            for soil_key in ["soil_from_asserted_coords", "soil_from_geo_loc_name"]:
                if soil_key in inferred:
                    ...
                    if soil_type:
                        source = "asserted coords" if "asserted" in soil_key else "geo_loc_name"
            ...
            if "publication_analysis" in inferred:
                ...
                click.echo(f"Study DOIs: {pub_data.get('total_dois', 0)} total, {pub_data.get('publication_doi_count', 0)} publications")
            
            # Show geospatial info from both sources
            for geo_key in ["geospatial_from_asserted_coords", "geospatial_from_geo_loc_name"]:
                if geo_key in inferred:
                    geo_data = inferred[geo_key]
                    source = "asserted coords" if "asserted" in geo_key else "geo_loc_name"
                    ...
                    if total_features > 0:
                        click.echo(f"Environmental features ({source}): {total_features} within {search_radius}m")
