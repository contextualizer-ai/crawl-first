...
def validate_unit_conversions(soil_props: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if all(prop in soil_data and soil_data[prop].get("value") for prop in ["sand_0_5cm", "silt_0_5cm", "clay_0_5cm"]):
        ...
        if not (90 <= total <= 110):  # Allow 10% tolerance
            validation["conversions_valid"] = False
            validation["issues"].append(f"Texture percentages sum to {total:.1f}% (expected ~100%)")
    ...
    if "soc_0_5cm" in soil_data and soil_data["soc_0_5cm"].get("value"):
        ...
        if not (0 <= soc <= 200):
            validation["conversions_valid"] = False
            validation["issues"].append(f"SOC out of range: {soc} g/kg (expected 0-200)")
...
def validate_single_point(point: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        validation = {
            ...
            "region": point["region"],
            "enrichment_successful": len(result["enrichment_summary"]["successful_enrichments"]),
            "total_enrichments": len(result["enrichment_summary"]["apis_used"]),
            "unit_validation": validate_unit_conversions(result.get("soil_properties", {})),
            "soil_data_available": result.get("soil_properties", {}).get("success", False),
            "classification_available": result.get("soil_classification", {}).get("success", False),
            "errors": []
        }
        ...
        soil_class = result.get("soil_classification", {}).get("soil_classification", {})
...
def validate_against_independent_sources(point: Dict[str, Any], result: Dict[str, Any]) -> Dict[str, Any]:
    """
    ...
    if point["region"] == "US":
        ...
        if sda_available and soilgrids_available:
            validation["independent_sources_checked"].append("USDA SDA vs ISRIC SoilGrids")
...
def run_validation(n_us: int = 5, n_global: int = 5) -> None:
    ...
    print(f"Running pipeline validation with {n_us} US points and {n_global} global points...")
    ...
    for point in test_points:
        ...
        if result.get("enrichment_successful", 0) > 0:
            try:
                ...
                from geospatial_enrichment import enrich_location
                enrichment_result = enrich_location(point["lat"], point["lon"], point["collection_date"])
                independent_validation = validate_against_independent_sources(point, enrichment_result)
        ...
        if results:
            ...
            for result in results:
                flattened = {
                    ...
                    "unit_conversions_valid": result.get("unit_validation", {}).get("conversions_valid", False),
                    "validation_errors": len(result.get("errors", [])),
                    "independent_sources_checked": len(result.get("independent_validation", {}).get("independent_sources_checked", [])),
                    "validation_issues": len(result.get("independent_validation", {}).get("validation_issues", []))
                }
    ...
    unit_validation_passed = sum(1 for r in results if r.get("unit_validation", {}).get("conversions_valid", False))
    
    print(f"\n=== VALIDATION SUMMARY ===")
    print(f"Total points tested: {total_points}")
    print(f"Successful enrichments (≥6/7): {successful_enrichments}/{total_points} ({100*successful_enrichments/total_points:.1f}%)")
    print(f"Unit validation passed: {unit_validation_passed}/{total_points} ({100*unit_validation_passed/total_points:.1f}%)")
    ...
    print(f"  - outputs/validation_results.json")
    print(f"  - outputs/validation_summary.csv")
