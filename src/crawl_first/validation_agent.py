...
class ValidationResult(BaseModel):
    ...
    env_triad_coherence: Optional[float] = Field(
        None,
        description="Coherence of env_broad_scale -> env_local_scale -> env_medium",
    )
...
class ValidationContext(RunContext):
    ...
    def __init__(self) -> None:
        ...
        self.coord_anomalies = {
            "zero_zero": (0.0, 0.0),
            "null_island_vicinity": [
                (lat, lon) for lat in range(-1, 2) for lon in range(-1, 2)
            ],
...
def create_validation_agent(
    ...
    return Agent(
        model,
        output_type=ValidationResult,  # type: ignore[arg-type]
...
def extract_relevant_slot_definitions(
    biosample_data: Dict[str, Any], schema: Dict[str, Any]
) -> str:
                ...
                with open(yaml_file, "r") as f:
                    ...
                    if other_study_id == study_id:
                        ...
                        sample_summary = {
                            ...
                            "env_broad_scale": asserted.get("env_broad_scale", {})
                            .get("term", {})
                            .get("name"),
                            "geo_loc_name": asserted.get("geo_loc_name", {}).get(
                                "has_raw_value"
                            ),
                        }
...
async def analyze_coordinate_consistency(
    ...
    if geo_loc_name:
        ...
        if "USA" in geo_loc_name and not (
            ...
            issues.append(
                f"USA location claimed but coordinates {asserted_lat}, {asserted_lon} outside US bounds"
            )
            geo_consistency_score = 0.2
        elif "Europe" in geo_loc_name and not (
            -15 <= asserted_lon <= 50 and 35 <= asserted_lat <= 75
        ):
            issues.append(
                "European location claimed but coordinates outside typical European bounds"
            )
            geo_consistency_score = 0.3
        elif "Australia" in geo_loc_name and not (
            110 <= asserted_lon <= 160 and -45 <= asserted_lat <= -10
        ):
            issues.append(
                "Australian location claimed but coordinates outside Australian bounds"
            )
            geo_consistency_score = 0.2
    ...
    if elevation is not None:
        if elevation < -500 and asserted_lat > 0:  # Northern hemisphere deep depression
            issues.append(
                f"Unusually low elevation {elevation}m for coordinates - check for data entry errors"
            )
            scores["elevation_plausibility"] = 0.4
        elif elevation > 6000:  # Very high elevation
            issues.append(
                f"Very high elevation {elevation}m - verify if mountainous region"
            )
...
async def validate_environmental_triad(
    ...
    if (
        env_broad_scale
        and "terrestrial biome" in env_broad_scale.lower()
        and env_medium
        and any(term in env_medium.lower() for term in ["water", "marine", "aquatic"])
    ):
    ...
    if (
        env_local_scale
        and "agricultural" in env_local_scale.lower()
        and env_medium
        and any(term in env_medium.lower() for term in ["forest", "woodland"])
    ):
    ...
    if ecosystem and ecosystem_category:
        ...
        if ecosystem == "Environmental" and ecosystem_category == "Terrestrial":
            ...
            ecosystem_score = 0.9  # Good consistency
        elif ecosystem == "Environmental" and ecosystem_category not in [
            "Terrestrial",
            "Aquatic",
            "Atmospheric",
        ]:
            issues.append(
                f"Environmental ecosystem with unexpected category: {ecosystem_category}"
            )
            ecosystem_score = 0.4
...
async def assess_soil_landcover_compatibility(
    ...
    if not soil_type or not land_cover_data:
        return {
            "scores": {},
            "issues": ["Missing soil type or land cover data"],
            "overall_score": 0.5,
        }
    ...
    for cover_type in dominant_covers:
        ...
        if soil_type in compatible_soils:
            ...
            issues.append(
                f"Soil type '{soil_type}' unusual for land cover '{cover_type}'"
            )
...
async def calculate_enrichment_coverage(
    ...
    present_sections = [
        section for section in expected_sections if section in inferred_data
    ]
    coverage_score = len(present_sections) / len(expected_sections)

    missing_sections = [
        section for section in expected_sections if section not in inferred_data
    ]
    ...
    return {
        ...
        "recommendations": [
            f"Consider enriching missing section: {section}"
...
async def run_llm_validation(
    ...
    # Handle case where output might be a string instead of ValidationResult
    if isinstance(validation_result, str):
        # Return a fallback ValidationResult if output is unexpectedly a string
        return ValidationResult(
            biosample_id=biosample_id,
            overall_score=0.0,
            coordinate_consistency=None,
            elevation_plausibility=None,
            ...
            issues=["LLM returned string instead of structured output"],
            recommendations=[f"LLM output: {validation_result}"],
        )
...
async def validate_directory(
    ...
    if resume and output_dir and output_dir.exists():
        ...
        if skipped_count > 0:
            print(
                f"[{datetime.now().strftime('%H:%M:%S')}] 📋 Resume mode: Skipping {skipped_count} already validated samples"
            )
    ...
    print(
        f"[{datetime.now().strftime('%H:%M:%S')}] Validating {total_files} biosample files..."
    )
    if output_dir:
        output_dir.mkdir(parents=True, exist_ok=True)
        print(
            f"[{datetime.now().strftime('%H:%M:%S')}] 💾 Streaming results to: {output_dir}"
        )
    ...
    for i, yaml_file in enumerate(yaml_files, 1):
        try:
            ...
            print(
                f"Progress: [{progress_bar}] {progress_percent}% ({i}/{total_files})",
                end="\r",
            )
...
@click.option(
    "--results-dir",
    default="archives/data/outputs/crawl-first/test-results/",
    help="Directory containing YAML biosample files to validate",
)
@click.option(
    "--max-samples", default=10, type=int, help="Maximum number of samples to validate"
)
@click.option("--model", default="anthropic/claude-sonnet", help="Model name to use")
@click.option(
    "--base-url",
...
@click.option(
    "--stream-dir",
    help="Directory to stream individual validation results (prevents data loss)",
)
@click.option(
    "--resume",
    is_flag=True,
    help="Resume validation by skipping already processed samples",
)
def main(
    results_dir: str,
    max_samples: int,
    ...
    # Prepare streaming directory path at function level
    stream_path = None
    if stream_dir:
    ...
    async def async_main() -> Union[int, List[ValidationResult]]:
        ...
        if nmdc_schema:
            ...
            click.echo(
                f"[{datetime.now().strftime('%H:%M:%S')}] ⚠️  NMDC schema not available - continuing without schema validation"
            )

        results_path = Path(results_dir)
        if not results_path.exists():
            click.echo(
                f"[{datetime.now().strftime('%H:%M:%S')}] ❌ Results directory not found: {results_path}"
            )
            return 1

        click.echo(f"[{datetime.now().strftime('%H:%M:%S')}] 🔬 Using model: {model}")
        click.echo(
            f"[{datetime.now().strftime('%H:%M:%S')}] 🌐 Using endpoint: {base_url}"
        )

        # Show streaming directory setup
        if stream_path:
            click.echo(
                f"[{datetime.now().strftime('%H:%M:%S')}] 💾 Streaming mode: Individual results will be saved to {stream_path}"
            )
            if resume:
                click.echo(
                    f"[{datetime.now().strftime('%H:%M:%S')}] 🔄 Resume mode enabled - will skip already validated samples"
                )

        # Run validation
        try:
            results = await validate_directory(
                results_path, max_samples, stream_path, resume
            )
        except Exception as e:
    ...
    if stream_path and not output and not results:
        ...
        click.echo(
            f"📊 Collected {len(results)} results from streaming directory for summary"
        )
    ...
    if stream_path:
        ...
        click.echo(
            f"💾 Individual results available in: {stream_path} ({individual_count} files)"
        )
        click.echo(
            f"💡 To resume validation later: --stream-dir {stream_path} --resume"
        )
