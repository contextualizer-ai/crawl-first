...
def fetch_teow(urls=TEOW_URLS, dest=DEST, timeout=120) -> str:
    ...
    raise RuntimeError(f"Failed to download TEOW from all sources. Last error: {last_err}")
...
def teow_read(zip_path=DEST) -> Tuple[gpd.GeoDataFrame, Dict[str, str]]:
    ...
    column_mapping = {
        "eco_name": pick_column("eco_name", "ecoregion", "name", "terrestrial_ecoregions"),
...
def teow_lookup_point(lon: float, lat: float, teow_gdf: gpd.GeoDataFrame, teow_cols: Dict[str, str]) -> Optional[Dict[str, Any]]:
    """
            ...
            if md5_path.exists():
                try:
                    ...
                    if current_md5 == stored_md5:
                        ...
                        print(f"⚠️ GeoPackage MD5 mismatch - will regenerate")
                        gpkg_path.unlink()
