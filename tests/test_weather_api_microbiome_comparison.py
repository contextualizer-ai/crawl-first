...
def call_open_meteo_comprehensive(lat: float, lon: float, date: str) -> Dict[str, Any]:
    ...
    pass4_params = [
        "vapour_pressure_deficit_mean", "cloudcover_mean",
        "precipitation_hours"
    ]
    ...
    for pass_name, param_set in parameter_sets:
        try:
            ...
            successful_params = len([k for k, v in pass_daily.items() if k != "time" and v and v[0] is not None])
    ...
    return {
        ...
        "coordinate_adjustment": {
            ...
            "data_quality": "excellent" if coord_distance_km < 1 else "good" if coord_distance_km < 5 else "moderate"
        },
...
def call_open_meteo_historical_forecast(lat: float, lon: float, date: str) -> Dict[str, Any]:
    ...
    daily_params = [
        ...
        "dewpoint_2m_mean", "vapour_pressure_deficit_max", 
    ...
    try:
        ...
        if "daily" in data and data["daily"]:
            ...
            return {"weather_data": {}, "success": False, "data_source": "Open-Meteo Historical Forecast API"}
...
def call_meteostat_comprehensive(lat: float, lon: float, date: str) -> Dict[str, Any]:
    ...
    try:
        ...
        stations = Stations()
        stations = stations.nearby(lat, lon, radius=200000)  # 200km radius for maximum coverage
        stations = stations.fetch(limit=20)  # Get top 20 closest stations for better analysis
        ...
        if not stations.empty:
            for idx, station in stations.iterrows():
                distance = haversine_distance(lat, lon, station.get('latitude', 0), station.get('longitude', 0))
                station_info = {
                    'id': idx,
                    ...
                    'data_period': {
                        'start': str(station.get('hourly_start')) if station.get('hourly_start') else None,
                        'end': str(station.get('hourly_end')) if station.get('hourly_end') else None
                    }
                }
        ...
        daily_params = len([col for col in daily_df.columns if not daily_df[col].isna().all()])
        hourly_params = len([col for col in hourly_df.columns if not hourly_df[col].isna().all()])
        ...
        data_completeness = {
            "daily_completeness": round((daily_params / max(1, len(daily_df.columns))) * 100, 1),
            "hourly_completeness": round((hourly_params / max(1, len(hourly_df.columns))) * 100, 1),
            "extended_period_days": len(extended_daily_df),
            "data_gaps": len(extended_daily_df) - len([i for i in extended_daily_df.index if not extended_daily_df.loc[i].isna().all()]),
            "daily_coverage": coverage_daily if 'coverage_daily' in locals() else None,
            "hourly_coverage": coverage_hourly if 'coverage_hourly' in locals() else None
        }
        
        # Add detailed parameter breakdown for microbiome research
        available_params = {
            "temperature": ["tavg", "tmin", "tmax"] if not daily_df.empty else [],
            "precipitation": ["prcp"] if not daily_df.empty and "prcp" in daily_df.columns else [],
            "wind": ["wspd", "wdir", "wpgt"] if not daily_df.empty else [],
            "pressure": ["pres"] if not daily_df.empty and "pres" in daily_df.columns else [],
            "sunshine": ["tsun"] if not daily_df.empty and "tsun" in daily_df.columns else [],
            "snow": ["snow"] if not daily_df.empty and "snow" in daily_df.columns else []
        }
        
        microbiome_relevant_count = sum(len(params) for params in available_params.values())
        ...
        return {
            ...
            "station_distance_km": round(closest_station_distance, 3) if closest_station_distance != float('inf') else None,
            "closest_station": {
                "id": closest_station_id,
                "distance_km": round(closest_station_distance, 3) if closest_station_distance != float('inf') else None,
                "data_quality": "excellent" if closest_station_distance < 5 else "good" if closest_station_distance < 25 else "moderate" if closest_station_distance < 100 else "poor"
            },
            ...
            "data_quality": {
                ...
                "spatial_representativeness": "high" if closest_station_distance < 10 else "medium" if closest_station_distance < 50 else "low",
...
def assess_microbiome_relevance(api_result: Dict[str, Any]) -> Dict[str, Any]:
    ...
    # Score each category
    critical_score = sum(1 for param_list in critical_params.values() 
                        if find_param_in_data(param_list, api_result))
    plant_score = sum(1 for param_list in plant_params.values() 
                     if find_param_in_data(param_list, api_result))
    microbial_score = sum(1 for param_list in microbial_params.values() 
                         if find_param_in_data(param_list, api_result))
...
class TestWeatherAPIMicrobiomeComparison:
    ...
    def test_microbiome_research_locations(self):
        ...
        for test_date in test_dates:
            ...
            for location in microbiome_locations:
                ...
                apis = [
                    ...
                    ("Open-Meteo Historical Forecast", call_open_meteo_historical_forecast),
                ...
                for api_name, api_func in apis:
                    ...
                    if result["success"]:
                        ...
                        if "Open-Meteo Historical Weather" in api_name:
                            ...
                            coord_quality = result.get("coordinate_adjustment", {}).get("data_quality", "unknown")
                            ...
                            station_quality = result.get("closest_station", {}).get("data_quality", "unknown")
                        ...
                        print(f"    🧬 Microbiome Relevance: {microbiome_assessment['relevance_score']}")
                        print(f"    📊 Critical/Plant/Microbial: {microbiome_assessment['critical_environmental_params']} | {microbiome_assessment['plant_host_biology_params']} | {microbiome_assessment['microbial_ecology_params']}")
                        ...
                        if api_name == "Open-Meteo" and "multi_pass_results" in result:
                            ...
                            for pass_result in result["multi_pass_results"]:
                                ...
                                print(f"      {status} {pass_result['pass_name']}: {pass_result['successful_params']}/{pass_result['requested_params']} params")
                        
                        location_results[api_name] = {
                            "success": True,
                            "total_parameters": total_params,
                            "microbiome_relevance": microbiome_assessment,
                            "distance_km": distance_km if 'distance_km' in locals() else None
                        }
                        
                        # Track API performance across locations and dates
                        if api_name not in api_summary:
                            api_summary[api_name] = {"successes": 0, "total_params": [], "relevance_scores": []}
                        api_summary[api_name]["successes"] += 1
                        api_summary[api_name]["total_params"].append(total_params)
                        api_summary[api_name]["relevance_scores"].append(microbiome_assessment["relevance_score"])
                        
                    else:
                        print(f"    ❌ Failed: {result.get('error', 'Unknown error')}")
                        location_results[api_name] = {"success": False, "error": result.get("error")}
        ...
        for api_name, summary in api_summary.items():
            if summary["successes"] > 0:
                ...
                avg_relevance = sum(summary["relevance_scores"]) / len(summary["relevance_scores"])
                
                print(f"\n📊 {api_name}:")
                print(f"   Success Rate: {summary['successes']}/{total_tests} tests ({len(test_dates)} dates × {len(microbiome_locations)} locations)")
                print(f"   Average Parameters: {avg_params:.1f}")
                print(f"   Average Microbiome Relevance: {avg_relevance:.3f}")
                print(f"   Parameter Range: {min(summary['total_params'])}-{max(summary['total_params'])}")
                print(f"   Relevance Range: {min(summary['relevance_scores']):.3f}-{max(summary['relevance_scores']):.3f}")
        
        print("\n🔬 MICROBIOME RESEARCH RECOMMENDATIONS:")
        print("1. Data Richness: Prioritize APIs with highest parameter counts")
        print("2. Environmental Coverage: Ensure critical microbiome parameters are included")
        print("3. Plant Host Biology: Important for rhizosphere and phyllosphere studies")
        print("4. Spatial Resolution: Consider coordinate precision for microhabitat studies")
        print("5. Temporal Resolution: Hourly data captures microbial response dynamics")
        
        # Ensure at least one API worked
        assert any(summary["successes"] > 0 for summary in api_summary.values()), "No weather APIs succeeded"
    ...
    def test_api_data_structure_analysis(self):
        ...
        test_location = {"lat": 40.7128, "lon": -74.0060, "name": "New York (Urban Microbiome)"}
        ...
        for api_name, api_func in apis:
            ...
            if result["success"]:
                        ...
                        for key, value in data.items():
                            if isinstance(value, (dict, list)) and len(str(value)) > 100:
                                ...
                                if current_depth < max_depth - 1:
                                    items.extend(analyze_data_structure(value, f"{prefix}  ", max_depth, current_depth + 1))
                            else:
                                items.append(f"{prefix}{key}: {type(value).__name__}")
                    elif isinstance(data, list) and data:
                        items.append(f"{prefix}[list of {len(data)} {type(data[0]).__name__} items]")
                        if isinstance(data[0], dict) and current_depth < max_depth - 1:
                            items.extend(analyze_data_structure(data[0], f"{prefix}  ", max_depth, current_depth + 1))
                ...
                print(f"  Critical Environmental: {relevance['critical_environmental_params']}")
    ...
    def test_api_cost_benefit_analysis(self):
        ...
        api_profiles = {
            "Open-Meteo": {
                ...
                "microbiome_strengths": ["Comprehensive soil parameters", "High temporal resolution", "Global coverage", "Plant biology data"],
            ...
            "Meteostat": {
                ...
                "microbiome_strengths": ["Station-based accuracy", "Long historical records", "Quality-controlled data"],
                "microbiome_gaps": ["Limited soil parameters", "Depends on station density"],
        ...
        for api_name, profile in api_profiles.items():
            ...
            print(f"  ✅ Microbiome Strengths: {', '.join(profile['microbiome_strengths'])}")
        ...
        print("2. 🥈 SECONDARY: Meteostat (free, high-quality station data for climate context)")
