...
class TestDirectElevationAPIComparison:
    ...
    def test_us_locations(self):
        ...
        us_locations = [
            {
                ...
                "lat": 44.428,
                "lon": -110.5885,
                "expected_range": (2450, 2470),
            },
            {
                "name": "Denver, CO",
                "lat": 39.7392,
                "lon": -104.9903,
                "expected_range": (1590, 1610),
            },
            {
            ...
            {
                "name": "Mount Whitney, CA",
                "lat": 36.5786,
                "lon": -118.2923,
                "expected_range": (4410, 4430),
            },
        ]
        ...
        for location in us_locations:
            ...
            assert (
                open_result["success"] is True
            ), f"Open Elevation failed for {location['name']}"

            # Google should succeed if API key is available
            if google_result["success"]:
                google_elev = google_result["elevation_meters"]
                assert (
                    min_elev <= google_elev <= max_elev
                ), f"Google elevation {google_elev}m outside expected range for {location['name']}"
            ...
            assert (
                min_elev <= usgs_elev <= max_elev
            ), f"USGS elevation {usgs_elev}m outside expected range for {location['name']}"
            assert (
                min_elev <= open_elev <= max_elev
            ), f"Open elevation {open_elev}m outside expected range for {location['name']}"

            # Difference should be reasonable (within 100m for most cases)
            diff = abs(usgs_elev - open_elev)
            if (
                location["name"] != "Mount Whitney, CA"
            ):  # High altitude may have more variation
                assert (
                    diff <= 100
                ), f"Large difference {diff}m between APIs for {location['name']}"
    ...
    def test_international_locations(self):
        ...
        for location in international_locations:
            ...
            assert (
                usgs_result["success"] is False
            ), f"USGS should fail for international location {location['name']}"
            assert (
                "coverage" in usgs_result.get("error", "").lower()
                or "bounds" in usgs_result.get("error", "").lower()
            )
            ...
            if google_result["success"]:
                ...
                assert (
                    open_google_diff <= max_diff
                ), f"Large difference {open_google_diff}m between Open and Google for {location['name']}"
    ...
    def test_ocean_locations(self):
        ...
        for location in ocean_locations:
            ...
            assert (
                open_result["elevation_meters"] <= 100
            ), f"Ocean elevation seems too high: {open_result['elevation_meters']}m"
            ...
            if google_result["success"]:
                ...
                if google_elev < 0:
                    print(
                        f"    ✓ Google correctly returns bathymetry: {google_elev}m below sea level"
                    )
                elif google_elev <= 50:
                    print(
                        f"    ~ Google returns near-sea-level: {google_elev}m (possibly shallow area)"
                    )
                else:
                    print(
                        f"    ? Google returns positive elevation: {google_elev}m (unexpected for deep ocean)"
                    )
    ...
    def test_invalid_coordinates(self):
        ...
        for coords in invalid_coords:
            ...
            if open_result["success"]:
                assert open_result["elevation_meters"] in [
                    0.0,
                    None,
                ], "Open Elevation should return 0 or null for invalid coords"

            # Google should properly validate and reject invalid coordinates
            if google_result["success"] is False:
                print("    ✓ Google properly rejects invalid coordinates")
                # Check for proper error messages
                error = google_result.get("error", "")
                if "INVALID_REQUEST" in error:
                    print("    ✓ Google returns proper INVALID_REQUEST error")
            else:
    ...
    def test_api_behavior_summary(self):
        ...
        print(
            "🌍 Best for global coverage: Google (includes bathymetry) > Open Elevation"
        )
        print("🆓 Best for free usage: USGS (US only) > Open Elevation (limited)")
        print(
            "🌊 Best ocean handling: Google (actual depths) > USGS (rejects) > Open (misleading 0.0m)"
        )
