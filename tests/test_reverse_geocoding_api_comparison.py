...
def call_nominatim_direct(lat: float, lon: float, zoom: int = 18) -> Dict[str, Any]:
    ...
    try:
        ...
        if data and "address" in data:
            ...
            address_fields = [
                "house_number", "road", "footway", "cycleway", "path", "pedestrian",
                "suburb", "district", "neighbourhood", "quarter", "city_district",
                "city", "town", "village", "hamlet", "municipality",
                "county", "state_district", "state", "province", "region",
                "postcode", "country", "country_code", "continent",
                "amenity", "building", "shop", "office", "leisure", "tourism",
                "historic", "natural", "landuse", "place", "man_made",
                "railway", "aeroway", "waterway", "boundary", "admin_level"
            ]
    ...
    try:
        ...
        if data.get("status") == "OK" and data.get("results"):
            ...
            if "administrative_area_level_1" in component_types:
                result["state"] = component_types["administrative_area_level_1"]["long_name"]
            if "administrative_area_level_2" in component_types:
                result["county"] = component_types["administrative_area_level_2"]["long_name"]
...
class TestReverseGeocodingAPIComparison:
    ...
    def test_diverse_global_locations(self):
        ...
        for location in test_locations:
            ...
            apis = [
                ("Nominatim (Cached)", lambda: get_reverse_geocoding_nominatim(lat, lon)),
                ("Nominatim (Direct)", lambda: call_nominatim_direct(lat, lon)),
                ("Google Reverse Geocoding", lambda: call_google_reverse_geocoding(lat, lon))
            ]
            ...
            for api_name, api_func in apis:
                ...
                try:
                    ...
                    if result.get("success"):
                        ...
                        print(f"    ✅ Success: {quality_assessment['completeness']} completeness")
                        print(f"    📍 Location: {result.get('display_name', 'N/A')}")
                        print(f"    🗺️  Quality Score: {quality_assessment['quality_score']}")
                        print(f"    📊 Fields: {quality_assessment['fields_present']}/{quality_assessment['total_fields']}")
                        
                        # Show key administrative fields
                        admin_fields = ["country", "state", "city", "county"]
                        admin_present = [field for field in admin_fields if result.get(field)]
                        if admin_present:
                        ...
                        if api_name not in api_summary:
                            api_summary[api_name] = {"successes": 0, "quality_scores": [], "completeness_levels": []}
                        api_summary[api_name]["successes"] += 1
                        api_summary[api_name]["quality_scores"].append(quality_assessment["quality_score"])
                        api_summary[api_name]["completeness_levels"].append(quality_assessment["completeness"])
        ...
        for api_name, summary in api_summary.items():
            if summary["successes"] > 0:
                avg_quality = sum(summary["quality_scores"]) / len(summary["quality_scores"])
                quality_range = f"{min(summary['quality_scores']):.3f}-{max(summary['quality_scores']):.3f}"
                
                print(f"\n📊 {api_name}:")
                print(f"   Success Rate: {summary['successes']}/{total_tests} locations")
                print(f"   Average Quality Score: {avg_quality:.3f}")
                print(f"   Quality Score Range: {quality_range}")
                print(f"   Completeness Levels: {', '.join(set(summary['completeness_levels']))}")
        
        print("\n🔬 BIOSAMPLE ENRICHMENT RECOMMENDATIONS:")
        print("1. Administrative Context: Country/State/County for regulatory classification")
        ...
        print("5. Caching: Cache results to respect rate limits and improve performance")
        
        # Ensure at least one API worked
        assert any(summary["successes"] > 0 for summary in api_summary.values()), "No reverse geocoding APIs succeeded"
    ...
    def test_zoom_level_comparison(self):
        ...
        for zoom in zoom_levels:
            ...
            result = call_nominatim_direct(test_location["lat"], test_location["lon"], zoom)
            ...
            if result["success"]:
                ...
                print(f"    📊 Fields: {quality['fields_present']}/{quality['total_fields']}")
        ...
        for zoom, data in zoom_results.items():
            if not data.get("failed"):
                print(f"   Zoom {zoom}: {data['quality_score']:.3f} quality, {data['fields_present']} fields")
    ...
    def test_api_rate_limiting_compliance(self):
        ...
        print(f"   Rate Compliance: {'✅ Good' if total_time >= expected_min_time else '⚠️ Too Fast'}")
        
        assert total_time >= expected_min_time - 0.5, "Rate limiting may be insufficient"
    ...
    def test_error_handling(self):
        ...
        for test_case in error_test_cases:
            ...
            print(f"   Cached Nominatim: {'✅ Success' if result.get('success') else '❌ Failed'}")
            
            # Test direct Nominatim
            result_direct = call_nominatim_direct(lat, lon)
            print(f"   Direct Nominatim: {'✅ Success' if result_direct.get('success') else '❌ Failed'}")
            
            # Verify error handling doesn't crash
            assert isinstance(result, dict), "Function should return dict even on error"
            assert isinstance(result_direct, dict), "Function should return dict even on error"